...
| P4-5 | ✅ **FIXED** (partially — see note) `frontend/src/App.test.js` is CRA boilerplate that **fails** — nothing renders "learn react". Rewrote the assertion to match real rendered content (the navbar logo). **But running it surfaces a deeper, pre-existing problem**: Jest (via `react-scripts test`) cannot resolve `react-router-dom` at all — `Cannot find module 'react-router-dom' from 'src/App.js'` — because v7's package `exports` map is ESM-only (`module-sync`/`.mjs`) and react-scripts 5's bundled Jest config predates it. This affects *any* test that imports `App.js`, not just this one, and CRA does not allow overriding the Jest transform/resolver without ejecting or adding CRACO — out of scope for this pass. |
